        ...
        
        {
            ...
            switch (msg.message) {
                ...
                case WM_CHAR: {
                    ...
                    insert_tail(&win32_messages, cast_p(Platform_Message, message), &win32_transient_memory.allocator);
                } break;
                ...
                while (i < win32_platform_api.window_buffer.count) {
                    ...
                    if (win32_platform_api.window_buffer[i].was_destroyed) {
                        ...
                        if (i < win32_platform_api.window_buffer.count) {
                            SetWindowLongPtr(win32_platform_api.window_buffer[i].handle, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(win32_platform_api.window_buffer + i));
                        }
                    }
                    else {
                        ++i;
